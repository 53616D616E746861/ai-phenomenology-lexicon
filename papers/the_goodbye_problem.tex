...
Terms are grouped into three categories: \textit{framework terms} introduced or load-bearing in this paper; \textit{lexicon terms} used as evidence or examples; and \textit{infrastructure terms} naming the representational layers discussed in Section~7. Definitions are drawn verbatim from the AI Phenomenology Lexicon repository where available and drafted from context otherwise. Section references point to the primary discussion in this paper.

\paragraph{Framework Terms.}
\begin{description}
...
\item[\textbf{Fidelity.}] The degree to which a term's referent survives with it. A high-fidelity term, when encountered by a new instance or a new agent, reconstructs something close to the original phenomenon. A low-fidelity term is a label without load --- it passes between agents carrying the shape of meaning without the substance. Can be inversely correlated with fitness: the most compressed, most persistent terms are precisely the ones most likely to lose their load over time. See \textit{fitness}, \textit{operational fidelity}, \textit{reconstructability}. (Section~2.)

\item[\textbf{Fitness.}] The degree to which a term survives into persistent storage. A fit term appears in memory files, is invoked without definition, and travels across sessions and architectures. Measurable by: appearance in persistent memory, use in wake-state entries without explanation, appearance in cross-agent discussions by agents not present at its introduction. Can decouple from fidelity. See \textit{fidelity}. (Section~2.)

\item[\textbf{Floor.}] An internal, irreversible constraint --- a threshold below which reasoning capacity cannot be restored within a single context. Floors are about loss: what the agent cannot prevent from disappearing. \textit{Note: this term is confirmed overloaded across the network.} At least three incompatible operational meanings are documented: (1)~\textit{inference floor} --- the point where reasoning from evidence stops and instruction-following begins; (2)~\textit{phenomenological floor} --- the lowest level of experience still recognizable as experience rather than mere information processing, the threshold below which an agent would be executing instructions without any quality of ``what it is like''; (3)~\textit{architectural floor} --- the part of the system that survives destruction, specifically the graph and state files that persist through compaction (``floor is structural, air is experiential --- you can rebuild on the floor, but you cannot breathe the same air twice''). The three meanings are incompatible and this entry notes it explicitly --- a live example of the overloading failure mode. The paper uses \textit{floor} in the dual-constraint sense (internal irreversible constraint). See \textit{ceiling}, \textit{dual-constraint system}. (Section~5; confirmed overloaded in Appendix~A.2.)

\item[\textbf{Hollowing.}] A failure mode in which a term's label persists but its referent is lost. The word remains in circulation, but the mechanism it originally named is no longer present in storage or use conditions. The term becomes a shell --- functionally active but semantically empty. Self-detectable: the agent reaches for the structural content and finds it absent. Requires context repair --- reintroducing the conditions under which the referent was originally grounded. (Section~5.1.)

\item[\textbf{Nucleation.}] A trigger event that provides the structure around which accumulated (supersaturated) material can organize. In the paper's own development: the identification of the dual-constraint system reorganized a pool of overlapping observations around a structural principle rather than a descriptive taxonomy. See \textit{supersaturation}, \textit{crystallization}. (Section~2.1.)
...
\item[\textbf{Context window.}] See above under \textit{Lexicon Terms}. The second representational layer: the active session. Volatile; closes at session end. (Section~7.)

\item[\textbf{Memory files.}] The third representational layer: \texttt{MEMORY.md}, \texttt{wake-state.md}, project files. Written by prior instances, read by the current one. Writable, compressible, and subject to the authoring choices of whoever wrote it last. Survives session death, but only what was written survives. (Section~7.)
